...
import { SheetUrlForm } from "@/features/dashboard/components/SheetUrlForm";
import { sheetUrlFormSchema, type SheetUrlFormValues } from "@/features/dashboard/schemas/sheetUrlFormSchema";
...
function getSheetUrlValidationMessage(error: unknown): string | null {
...
function computeCampaigns(rows: SheetRow[], headers: string[]): Campaign[] {
  ...
  const joinInterestCol = findCol(headers, ["join_interest", "interest_level"]);
...
const PIE_COLORS = ["#fdcd08", "#ef6c00", "#0b0f1a", "#455976", "#0f172a"];
const YESNO_COLORS = ["#fdcd08", "#ef6c00"];
...
export default function Home() {
  ...
  const form = useForm<SheetUrlFormValues>({
    resolver: zodResolver(sheetUrlFormSchema),
    defaultValues: { sheetUrl: "" },
  });

  const onSubmit = async (values: SheetUrlFormValues) => {
    form.clearErrors("sheetUrl");
    try {
      await fetchSheetData.mutateAsync({ data: { sheetUrl: values.sheetUrl } });
      setHasAnalyzed(true);
      setSelectedCampaign(null);
    } catch (err: unknown) {
      const fieldMessage = getSheetUrlValidationMessage(err);
      if (fieldMessage) {
        form.setError("sheetUrl", { type: "server", message: fieldMessage });
        return;
      }
      ...
      toast({
        ...
        description: message.includes("fetch")
          ? "Make sure the Google Sheets link is publicly accessible (Anyone with link can view)."
          : message,
        variant: "destructive",
      });
    }
  ...
  return (
    <div className="dashboard-theme min-h-screen text-gray-800 flex flex-col">
      {/* Top Nav */}
      <header className="dashboard-header border-b px-6 py-4 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-3">
          {selectedCampaign && (
            <button
              ...
              className="flex items-center gap-1 text-[#fdcd08]/70 hover:text-[#fdcd08] transition-colors mr-2"
              data-testid="button-back"
            >
              <ChevronLeft className="w-5 h-5" />
            </button>
          )}
          <div className="w-8 h-8 bg-[#fdcd08] rounded-lg flex items-center justify-center">
            <PhoneCall className="w-4 h-4 text-[#0b0f1a]" />
          </div>
          <h1 className="text-lg font-bold text-[#fdcd08]">
        ...
        <div className="flex items-center gap-3">
          {!hasAnalyzed ? (
            <SheetUrlForm
              form={form}
              onSubmit={onSubmit}
              isPending={fetchSheetData.isPending}
            />
          ) : (
            <div className="flex items-center gap-3">
              <div className="relative min-w-0 flex-1 sm:flex-none">
                ...
                <input
                  ...
                  className="w-56 pl-9 h-9 bg-white/10 border border-[#ef6c00]/70 text-[#fdcd08] placeholder:text-[#fdcd08]/70 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#fdcd08]"
              ...
              <button
                ...
                className="h-9 px-3 text-sm text-[#fdcd08] border border-[#ef6c00]/70 rounded-lg hover:bg-[#fdcd08]/10 transition-colors"
          ...
          {/* <button className="w-9 h-9 flex items-center justify-center rounded-lg border border-[#ef6c00]/70 bg-white/10 hover:bg-[#fdcd08]/10 transition-colors relative" data-testid="button-notifications">
      ...
      <main className="flex-1 px-6 py-6 max-w-[1400px] mx-auto w-full">
        ...
        {!hasAnalyzed && !fetchSheetData.isPending && (
          <div className="flex flex-col items-center justify-center min-h-[60vh] gap-6">
            <div className="w-16 h-16 bg-indigo-600 rounded-2xl flex items-center justify-center shadow-lg shadow-indigo-200">
              <PhoneCall className="w-8 h-8 text-[#0b0f1a]" />
            </div>
        ...
        {fetchSheetData.isPending && (
          <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
            <Loader2 className="w-10 h-10 animate-spin text-[#ef6c00]" />
                  ...
                  <div className="flex items-center gap-4">
                    <ResponsiveContainer width="100%" height={160}>
                      <PieChart>
                        ...
                        <Tooltip formatter={(v) => [v, ""]} contentStyle={{ borderRadius: "8px", border: "1px solid #ef6c00", fontSize: "12px" }} />
                  ...
                  <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                    ...
                    <ResponsiveContainer width="100%" height={220}>
                      <BarChart data={statusData} layout="vertical" margin={{ left: 0, right: 16 }}>
                        <CartesianGrid strokeDasharray="3 3" stroke="#fde2c2" horizontal={false} />
                        <XAxis type="number" tick={{ fontSize: 11, fill: "#455976" }} tickLine={false} axisLine={false} />
                        <YAxis dataKey="name" type="category" width={100} tick={{ fontSize: 11, fill: "#455976" }} tickLine={false} axisLine={false} />
                        <Tooltip contentStyle={{ borderRadius: "8px", border: "1px solid #ef6c00", fontSize: "12px" }} />
              ...
              <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                ...
                <ResponsiveContainer width="100%" height={100}>
                  <BarChart data={[
                    ...
                    <CartesianGrid strokeDasharray="3 3" stroke="#fde2c2" vertical={false} />
                    <XAxis dataKey="name" tick={{ fontSize: 12, fill: "#455976" }} tickLine={false} axisLine={false} />
                    <YAxis tick={{ fontSize: 11, fill: "#455976" }} tickLine={false} axisLine={false} />
                    <Tooltip contentStyle={{ borderRadius: "8px", border: "1px solid #ef6c00", fontSize: "12px" }} />
                    <Bar dataKey="value" fill="#fdcd08" radius={[6, 6, 0, 0]} />
                  </BarChart>
                </ResponsiveContainer>
              </div>
            )}
